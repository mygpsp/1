...
class Writer(asyncio.Protocol):
    def connection_made(self, transport):
        """Store the serial transport and schedule the task to send 
data.
        """
...
reader = serial_asyncio.create_serial_connection(loop, Reader, '/dev/ttyUSB0', 
baudrate=115200)
writer = serial_asyncio.create_serial_connection(loop, Writer, '/dev/ttyUSB0', 
baudrate=115200)
asyncio.ensure_future(reader)
print('Reader scheduled')
asyncio.ensure_future(writer)
print('Writer scheduled')
loop.call_later(99999999999, loop.stop)
loop.run_forever()
print('Done')
